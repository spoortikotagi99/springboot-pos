...
public class DALManager implements IDALManager {
    ...
    public void verifyUser(UserDTO user, Response responseObj) {
        ...
        if (connection == null) {
            ...
            responseObj.getMessagesList().add(message);  // ✅ Use the getter method
    ...
    public void addUser(UserDTO userObj, Response responseObj) {
        ...
        if (connection == null) {
            Message message = new Message("Database Connection issue please contact customer services.", MessageType.Exception);
            responseObj.getMessagesList().add((message));
        } else {
    ...
    public ArrayList<UserDTO> getUsers(Response response) {
        ...
        if (connection == null) {
            Message message = new Message("Database Connection issue please contact customer services.", MessageType.Exception);
            response.getMessagesList().add((message));
        }
        ResultSet resultSet = null;
    ...
    public ArrayList<SupplierDTO> getSuppliers(Response res) {
        ...
        if (connection == null) {
            Message message = new Message("Database Connection issue please contact customer services.", MessageType.Exception);
            res.getMessagesList().add((message));
        }
        ResultSet resultSet = null;
    ...
    public List<SalesReportDTO> getMonthlySales(Connection connection, int month, int year) {
        List<SalesReportDTO> salesReportList = new ArrayList<>();
        ResultSet resultSet = objReader.getMonthlySales(connection,month, year);


        try {
            while (resultSet != null && resultSet.next()) {
                salesReportList.add(new SalesReportDTO(
                        resultSet.getString("sale_date"),        // Date (String)
                        resultSet.getDouble("total_sales"), // Total Sales Amount (double)
                        resultSet.getInt("total_transactions") // Total Transactions (int)
                ));
    ...
    /**
     * Retrieves the monthly salary report and converts it into a List of SalaryReportDTO.
     */
    public List<SalaryReportDTO> getMonthlySalaries(Connection connection, int month, int year) {
        List<SalaryReportDTO> salaryReportList = new ArrayList<>();
        ResultSet resultSet = objReader.getMonthlySalaries(connection, month, year);


        try {
            while (resultSet != null && resultSet.next()) {
                salaryReportList.add(new SalaryReportDTO(
                        resultSet.getString("name"),        // Employee Name
                        resultSet.getDouble("salary"),      // Salary Amount
                        resultSet.getString("payment_date") // Payment Date
                ));
            }
        } catch (Exception e) {
            e.printStackTrace();
